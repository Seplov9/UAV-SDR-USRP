...
% 读取CSV文件
% file_path = 'usrp240107/';
% data_name = ['没人/3.2G-4M-1.xlsx'];
file_path = 'usrp240308/';
data_name = ['150m/N/data1.xlsx'];
...
for i=1:size(CFR0_LS, 2)
    
    CFR_TEMP = CFR0_LS(:,i) .* conj(CFR1_LS(:,i));
...
%%
X=CFRX(data_station(1:end),:);

save(fullfile('usrp240308/150m', 'CFR8.mat'),'X');

%%
% figure;
% dataxx=squeeze(CFRX(:,1:1000));
% data_frame0real = real(dataxx);
% data_frame0imag = imag(dataxx);
% plot(data_frame0real(9,:));
% hold on;
% plot(data_frame0imag(9,:));
% figure;
% p_data=abs(dataxx);
% plot(p_data(9,:));
